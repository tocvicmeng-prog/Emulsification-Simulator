...
def plot_modulus_comparison(m: MechanicalResult) -> go.Figure:
    """Bar chart comparing G_agarose, G_chitosan, and G_DN with HS reference lines."""
    ...
    # Add horizontal dashed reference lines for HS bounds (single-phase composite ref)
    if _hs_lo > 0:
        fig.add_hline(
            y=_hs_lo / 1000,
            line_dash="dash",
            line_color="gray",
            annotation_text="HS lower (ref)",
    ...
    if _hs_lo > 0 and _hs_hi > 0:
        _ref_text = f"  |  Single-phase ref: [{_hs_lo/1000:.1f}, {_hs_hi/1000:.1f}] kPa"
    fig.update_layout(
        title=f"Modulus Breakdown (L4) — Model: {_model_label}{_ref_text}",
