...
def react_edc_nhs_two_step(
    *,
    ...
    # any P that came from reactivation cycles. For reporting we use a
    # simpler proxy: fraction of C0 not in {A_f + E_f + P_f} = C_f / C0.
    # This underestimates total hydrolysis events but gives the "yield
    # loss relative to product" view that M2/L3 callers need.
    p_residual_cooh = max(C_f / c_cooh_initial, 0.0)
